...
public class DifferApplication {
	...
	public static void main(String[] args) throws Exception {
		...
		for (String relativePath : baselineFiles.keySet()) {
		...
		for (String relativePath : baselineFiles.keySet()) {
			...
			Path baselineFile = baselineFiles.get(relativePath);
			Path targetFile = targetFiles.get(relativePath);

			if (!Files.isRegularFile(baselineFile) || !Files.isRegularFile(targetFile)) {
				System.out.println("SKIPPED (not a file): " + relativePath);
				continue;
			}

			String hashA = FileHasher.sha256(baselineFile);
			String hashB = FileHasher.sha256(targetFile);

			if (hashA.equals(hashB)) {
				System.out.println("UNCHANGED: " + relativePath);
			} else {
				System.out.println("MODIFIED: " + relativePath);

				FileDiffResult diff = LineDiffService.diffFiles(baselineFile, targetFile, Paths.get(relativePath));

				if (diff.isEmpty()) {
					System.out.println("  (no line-level changes)");
				} else {
